...
## The social preview image (`docs/og-image.jpg`)

A static asset, not regenerated by the scheduled job (which writes
`docs/index.html`, `docs/feed.xml`, `docs/badges/*.json`, and `data/`, but
never touches this file). It deliberately
does **not** bake in any live scan numbers -- an earlier version showed a
specific severity-count breakdown from whatever the tracked list looked
like on the day it was made, which was accurate then and silently wrong
within a week once the crate list and scan results moved on, with no
mechanism to notice or fix it. The current version only shows things that
change by deliberate edit (the branding, the category labels), not things
that drift automatically. If you regenerate it, keep that constraint:
either make it truly static, or wire its generation into the same run
that writes everything else, so it can't fall out of sync silently again.

## Embeddable badges

Every tracked crate gets a small JSON file at
`docs/badges/<crate-name>.json`, in
[shields.io's endpoint-badge schema](https://shields.io/endpoint) --
shields.io fetches it and renders the actual badge image, so there's no
hand-rolled SVG text-metrics code here to get subtly wrong. Embed one in
any README:

```markdown
[![capscan](https://img.shields.io/endpoint?url=https://poglesbyg.github.io/capscan-leaderboard/badges/tokio.json)](https://poglesbyg.github.io/capscan-leaderboard/)
```

Swap `tokio` for any name in [`crates.txt`](crates.txt). Red ("critical")
means at least one high-severity signal is present on the crate's
currently-tracked version, orange means medium but no high, green means
neither. Regenerated every run alongside everything else, and pruned the
same way `data/snapshot.json` is: removing a crate from `crates.txt`
deletes its badge file too, rather than leaving a stale one live forever.
